      ...
      <section className="mb-8">
        <div className="flex items-end justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-2">Open Ad Code on a listing, choose a slot size, then paste that code where that size should appear on your website.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="border border-border p-8 sm:p-10 text-center">
            ...
            {stripeConnected ? <Link to="/listings/new" className="inline-block mt-4 text-primary text-xs uppercase tracking-[0.3em]">Create your first listing →</Link> : <button onClick={startStripeConnect} disabled={connecting} className="inline-block mt-4 text-primary text-xs uppercase tracking-[0.3em] disabled:opacity-60">Connect Stripe before listing →</button>}
          </div>
        ) : (
          <div className="border border-border bg-card overflow-hidden" data-testid="owner-listings-table">
                    ...
                    <React.Fragment key={listing.id}>
                      <tr className="border-b border-border last:border-b-0">
                        ...
                        <td className="p-3 text-right space-x-2">
                          <button onClick={() => setOpenCodeListingId(openCodeListingId === listing.id ? '' : listing.id)} className="inline-flex items-center gap-1 border border-acid text-acid px-2 py-1 text-[10px] uppercase tracking-[0.2em] hover:bg-acid hover:text-black" data-testid={`open-ad-code-${listing.id}`}>
                            <Code2 size={10}/> Ad Code
                          </button>
                      ...
                      {openCodeListingId === listing.id && (
                        <tr className="border-b border-border">
                          <td colSpan="5" className="p-4 bg-background">
                            <AdCodePanel listing={listing} />
                          </td>
                        </tr>
                      )}
                    </React.Fragment>
            ...
            <div className="md:hidden divide-y divide-border">
              {listings.map((listing) => <ListingCard key={listing.id} listing={listing} toggleStatus={toggleStatus} openCodeListingId={openCodeListingId} setOpenCodeListingId={setOpenCodeListingId} />)}
            </div>
          </div>
        )}
      </section>

      <SalesHistory orders={salesData.orders} actionBusy={actionBusy} approveOrder={approveOrder} denyOrder={denyOrder} />
    </div>
  );
}
  ...
  return (
    <div className="border border-acid bg-acid/5 p-4" data-testid={`ad-code-panel-${listing.id}`}>
      ...
      <p className="text-sm text-muted-foreground leading-relaxed mb-4">
        Choose the ad size you want on your website. Copy that code and paste it exactly where that size should appear. You only install each slot once.
      </p>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {AD_SIZES.map((size) => {
          const code = getEmbedCode(listing, size);
          return (
            <div key={size} className="border border-border bg-black p-3">
              <div className="flex items-center justify-between gap-3 mb-2">
                <div className="font-mono text-sm text-acid">{size}</div>
                <button onClick={() => copyText(code, `${size} ad code copied`)} className="inline-flex items-center gap-1 border border-acid text-acid px-2 py-1 text-[10px] uppercase tracking-[0.2em] hover:bg-acid hover:text-black" data-testid={`copy-ad-code-${listing.id}-${size}`}>
                  <Copy size={10}/> Copy
                </button>
              </div>
              <code className="block text-[10px] leading-relaxed font-mono text-muted-foreground break-all">{code}</code>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="border border-gold bg-card p-5" data-testid={`pending-review-${order.id}`}>
      ...
      <div className="mb-4">
        ...
        {creatives.length === 0 ? <div className="border border-primary text-primary p-4 text-sm">No creative previews found for this order.</div> : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {creatives.map((creative) => <CreativePreview key={creative.id} creative={creative} />)}
          </div>
        )}
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button onClick={() => approveOrder(order)} disabled={Boolean(actionBusy)} className="border border-acid text-acid py-3 text-xs uppercase tracking-[0.3em] font-bold hover:bg-acid hover:text-black disabled:opacity-60">{actionBusy === `approve-${order.id}` ? 'Approving...' : 'Approve Ad'}</button>
        <button onClick={() => denyOrder(order)} disabled={Boolean(actionBusy)} className="border border-primary text-primary py-3 text-xs uppercase tracking-[0.3em] font-bold hover:bg-primary hover:text-primary-foreground disabled:opacity-60">{actionBusy === `deny-${order.id}` ? 'Denying...' : 'Deny + Refund'}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-card p-4" data-testid={`owner-listing-card-${listing.id}`}>
      ...
      <div className="mt-4 grid grid-cols-1 gap-2">
        <button onClick={() => setOpenCodeListingId(open ? '' : listing.id)} className="w-full inline-flex items-center justify-center gap-2 border border-acid text-acid px-3 py-3 text-[10px] uppercase tracking-[0.25em] hover:bg-acid hover:text-black"><Code2 size={12}/> Ad Code</button>
        ...
        <Link to={`/listings/${listing.id}/edit`} className="w-full inline-flex items-center justify-center gap-2 border border-border px-3 py-3 text-[10px] uppercase tracking-[0.25em] hover:border-primary hover:text-primary"><Edit3 size={12}/> Edit Listing</Link>
      </div>
      {open && <div className="mt-4"><AdCodePanel listing={listing} /></div>}
    </div>
  );
}

function SalesHistory({ orders, actionBusy, approveOrder, denyOrder }) {
  if (orders.length === 0) return <div className="border border-border p-8 sm:p-12 text-center text-sm text-muted-foreground" data-testid="owner-no-sales">No sales yet.</div>;
            ...
            {orders.map((order) => (
              <tr key={order.id} className="border-b border-border last:border-b-0">
                ...
                <td className="p-3 text-right">
                  {order.payment_status === 'paid' && ['pending', 'awaiting_approval'].includes(order.approval_status) ? <div className="flex justify-end gap-2"><button onClick={() => approveOrder(order)} disabled={Boolean(actionBusy)} className="border border-acid text-acid px-2 py-1 text-[10px] uppercase tracking-[0.2em] disabled:opacity-60">Approve</button><button onClick={() => denyOrder(order)} disabled={Boolean(actionBusy)} className="border border-primary text-primary px-2 py-1 text-[10px] uppercase tracking-[0.2em] disabled:opacity-60">Deny</button></div> : <span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">—</span>}
                </td>
              </tr>
            ))}
          </tbody>
