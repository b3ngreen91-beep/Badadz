...
export default function OwnerDashboard() {
  ...
  const launchOrders = showTestRecords ? allOrders : allOrders.filter((o) => !isTestOrder(o));
  const approvedLaunchOrders = launchOrders.filter((o) => o.payment_status === 'paid' && o.approval_status === 'approved');
  const allApprovedOrders = allOrders.filter((o) => o.payment_status === 'paid' && o.approval_status === 'approved');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5 sm:py-8" data-testid="owner-dashboard">
      ...
      {hiddenTestCount > 0 && (
        <div className="border border-border bg-card p-4 mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <p className="text-xs text-muted-foreground leading-relaxed">
            {showTestRecords ? 'Showing $0 test records in launch stats. Sales History always shows all records.' : `${hiddenTestCount} $0 test record${hiddenTestCount === 1 ? '' : 's'} hidden from launch stats. Sales History still shows all records.`}
          </p>
          <button onClick={() => setShowTestRecords((v) => !v)} className="border border-border px-4 py-2 text-[10px] uppercase tracking-[0.25em] hover:border-primary hover:text-primary">
            {showTestRecords ? 'Hide Test From Stats' : 'Include Test In Stats'}
          </button>
        </div>
      )}
      ...
      <section className="mb-8">
        ...
        {loading ? <div className="text-muted-foreground text-xs uppercase tracking-[0.3em] py-8">Loading...</div> : <ListingsTable listings={visibleListings} approvedOrders={allApprovedOrders} openCodeListingId={openCodeListingId} setOpenCodeListingId={setOpenCodeListingId} toggleStatus={toggleStatus} />}
      </section>
    ...
    <div className="border border-border bg-card overflow-hidden" data-testid="owner-listings-table">
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>{listings.map((listing) => <ListingRows key={listing.id} listing={listing} approvedOrders={approvedOrders} open={openCodeListingId === listing.id} setOpenCodeListingId={setOpenCodeListingId} toggleStatus={toggleStatus} />)}</tbody>
        </table>
      </div>
      <div className="md:hidden divide-y divide-border">{listings.map((listing) => <ListingCard key={listing.id} listing={listing} approvedOrders={approvedOrders} toggleStatus={toggleStatus} openCodeListingId={openCodeListingId} setOpenCodeListingId={setOpenCodeListingId} />)}</div>
    </div>
  );
}

function ListingRows({ listing, approvedOrders, open, setOpenCodeListingId, toggleStatus }) {
    ...
    <React.Fragment>
      <tr className="border-b border-border last:border-b-0">
        ...
        <td className="p-3 text-right space-x-2">
          {showCode && <button onClick={() => setOpenCodeListingId(open ? '' : listing.id)} className="inline-flex items-center gap-1 border border-acid text-acid px-2 py-1 text-[10px] uppercase tracking-[0.2em] hover:bg-acid hover:text-black"><Code2 size={10}/> Ad Code</button>}
          {listing.status !== 'sold' && <button onClick={() => toggleStatus(listing)} className="inline-flex items-center gap-1 border border-border px-2 py-1 text-[10px] uppercase tracking-[0.2em] hover:border-primary hover:text-primary">{listing.status === 'active' ? <><Pause size={10}/> Pause</> : <><Play size={10}/> Activate</>}</button>}
          <Link to={`/listings/${listing.id}/edit`} className="inline-flex items-center gap-1 border border-border px-2 py-1 text-[10px] uppercase tracking-[0.2em] hover:border-primary hover:text-primary"><Edit3 size={10}/> Edit</Link>
        </td>
      </tr>
      {showCode && open && <tr className="border-b border-border"><td colSpan="5" className="p-4 bg-background"><AdCodePanel listing={listing} /></td></tr>}
    </React.Fragment>
  );
}
...
function CampaignAnalytics({ orders }) {
  ...
  return <section className="mb-8"><h2 className="font-display font-black uppercase text-2xl sm:text-3xl tracking-tight mb-4">Campaign Analytics</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{sorted.slice(0, 4).map((order) => { const v = Number(order.impression_count || 0); const c = Number(order.click_count || 0); return <div key={order.id} className="border border-border bg-card p-5"><div className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground mb-2">{order.website_name}</div><div className="grid grid-cols-3 gap-px bg-border border border-border"><MiniStat label="Views" value={v.toLocaleString()} /><MiniStat label="Clicks" value={c.toLocaleString()} /><MiniStat label="CTR" value={ctrPercent(v, c)} /></div><p className="text-xs text-muted-foreground mt-3">CTR means click-through rate: clicks divided by views.</p></div>; })}</div></section>;
}
...
function ListingCard({ listing, approvedOrders, toggleStatus, openCodeListingId, setOpenCodeListingId }) {
  ...
  const showCode = canShowAdCode(listing, approvedOrders);
  return <div className="bg-card p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><Link to={`/listings/${listing.id}`} className="block font-display font-black uppercase text-xl tracking-tight hover:text-primary truncate">{listing.website_name}</Link><div className="mt-2 text-[10px] uppercase tracking-[0.25em] text-muted-foreground">{listing.category || 'Uncategorized'}</div></div><StatusBadge status={listing.status} live={hasLiveCampaign(listing, approvedOrders)} /></div><div className="mt-5 grid grid-cols-2 gap-px bg-border border border-border"><MiniStat label="Price" value={`$${Number(listing.monthly_price || 0).toLocaleString()}`} /><MiniStat label="Status" value={listing.status === 'sold' && hasLiveCampaign(listing, approvedOrders) ? 'campaign live' : listing.status} /></div><div className="mt-4 grid grid-cols-1 gap-2">{showCode && <button onClick={() => setOpenCodeListingId(open ? '' : listing.id)} className="w-full inline-flex items-center justify-center gap-2 border border-acid text-acid px-3 py-3 text-[10px] uppercase tracking-[0.25em] hover:bg-acid hover:text-black"><Code2 size={12}/> Ad Code</button>}{listing.status !== 'sold' && <button onClick={() => toggleStatus(listing)} className="w-full inline-flex items-center justify-center gap-2 border border-border px-3 py-3 text-[10px] uppercase tracking-[0.25em] hover:border-primary hover:text-primary">{listing.status === 'active' ? <><Pause size={12}/> Pause Listing</> : <><Play size={12}/> Activate Listing</>}</button>}<Link to={`/listings/${listing.id}/edit`} className="w-full inline-flex items-center justify-center gap-2 border border-border px-3 py-3 text-[10px] uppercase tracking-[0.25em] hover:border-primary hover:text-primary"><Edit3 size={12}/> Edit Listing</Link></div>{showCode && open && <div className="mt-4"><AdCodePanel listing={listing} /></div>}</div>;
}
...
function SalesHistory({ orders, actionBusy, approveOrder, denyOrder }) {
  ...
  return <section><h2 className="font-display font-black uppercase text-2xl sm:text-3xl tracking-tight mt-10 mb-4">Sales History</h2><p className="text-xs text-muted-foreground mb-3">Sales History shows every order, including $0 tests. Launch stats above can hide test records.</p><div className="border border-border bg-card overflow-x-auto"><table className="w-full text-sm min-w-[1100px]"><thead className="border-b border-border bg-secondary"><tr className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground"><th className="text-left p-3">Date</th><th className="text-left p-3">Listing</th><th className="text-left p-3">Advertiser</th><th className="text-right p-3">Paid</th><th className="text-right p-3">You Earn</th><th className="text-right p-3">Views</th><th className="text-right p-3">Clicks</th><th className="text-right p-3">CTR</th><th className="text-left p-3">Payment</th><th className="text-left p-3">Approval</th><th className="text-left p-3">Payout</th><th className="text-right p-3">Actions</th></tr></thead><tbody>{orders.map((order) => { const v = Number(order.impression_count || 0); const c = Number(order.click_count || 0); return <tr key={order.id} className="border-b border-border last:border-b-0"><td className="p-3 font-mono text-xs text-muted-foreground">{new Date(order.created_at).toLocaleDateString()}</td><td className="p-3">{order.website_name}{isTestOrder(order) && <span className="ml-2 text-[9px] uppercase tracking-[0.2em] text-muted-foreground">test</span>}</td><td className="p-3 text-muted-foreground">{order.advertiser_name}<br/><span className="text-xs">{order.advertiser_email}</span></td><td className="p-3 text-right font-mono">${Number(order.price_paid || 0).toFixed(2)}</td><td className="p-3 text-right font-mono text-acid">${Number(order.seller_earnings || 0).toFixed(2)}</td><td className="p-3 text-right font-mono">{v.toLocaleString()}</td><td className="p-3 text-right font-mono">{c.toLocaleString()}</td><td className="p-3 text-right font-mono">{ctrPercent(v, c)}</td><td className="p-3"><StatusText value={order.payment_status} /></td><td className="p-3"><StatusText value={order.approval_status || 'awaiting_payment'} /></td><td className="p-3"><PayoutStatus order={order} /></td><td className="p-3 text-right">{order.payment_status === 'paid' && ['pending', 'awaiting_approval'].includes(order.approval_status) ? <div className="flex justify-end gap-2"><button onClick={() => approveOrder(order)} disabled={Boolean(actionBusy)} className="border border-acid text-acid px-2 py-1 text-[10px] uppercase tracking-[0.2em] disabled:opacity-60">Approve</button><button onClick={() => denyOrder(order)} disabled={Boolean(actionBusy)} className="border border-primary text-primary px-2 py-1 text-[10px] uppercase tracking-[0.2em] disabled:opacity-60">Deny</button></div> : <span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">—</span>}</td></tr>; })}</tbody></table></div></section>;
}
