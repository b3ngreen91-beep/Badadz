...
export default function Home() {
  ...
  const stats = useMemo(() => {
    const prices = listings.map((l) => Number(l.monthly_price || 0)).filter(Number.isFinite);
    return {
      activeListings: listings.length,
      categoryCount: categories.length || new Set(listings.map((l) => l.category).filter(Boolean)).size,
      avgPrice: prices.length ? Math.round(prices.reduce((sum, price) => sum + price, 0) / prices.length) : 0,
      startingAt: prices.length ? Math.min(...prices) : 0,
    };
  }, [listings, categories]);
  ...
  return (
    <div data-testid="home-page">
      <section className="relative border-b border-border overflow-hidden" style={{ backgroundImage: `linear-gradient(rgba(0,0,0,0.82), rgba(0,0,0,0.96)), url(${HERO_BG})`, backgroundSize: 'cover', backgroundPosition: 'center' }} data-testid="hero">
      ...
      <section className="border-b border-border bg-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-px bg-border border-x border-border text-center">
          {['Secure Stripe payments', 'Owner-approved ads', 'Automatic ad serving', 'Views/clicks tracking'].map((item) => <div key={item} className="bg-background p-3 text-[10px] uppercase tracking-[0.22em] text-muted-foreground">{item}</div>)}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-14 border-b border-border" data-testid="categories-section">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-border border border-border mt-6">
          {displayedCategories.map((c) => <button key={c.name} onClick={() => selectCategory(c.name)} className={`bg-background p-4 text-left hover:bg-card transition-colors ${category === c.name ? 'text-primary' : 'text-foreground'}`}><div className="font-display font-black uppercase text-lg tracking-tight">{c.name}</div><div className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground mt-1">{c.count ? `${c.count} live` : 'coming soon'}</div></button>)}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16 border-b border-border" data-testid="featured-section">
        ...
        {featuredListings.length === 0 ? <div className="border border-border bg-card p-8 text-center text-sm text-muted-foreground">No featured placements yet. Be the first website owner to list ad inventory.</div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{featuredListings.map((listing) => <ListingCard key={listing.id} listing={listing} />)}</div>}
      </section>

      <section id="marketplace" className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16 border-b border-border">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6 sm:mb-8"><SectionHeader eyebrow="[ 01 / Marketplace ]" title="Live ad placements." compact /><div className="flex items-center gap-2 text-xs text-muted-foreground uppercase tracking-[0.2em]"><SlidersHorizontal size={14} /> {listings.length} placements</div></div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-3 mb-8 border border-border p-4 bg-card" data-testid="filters">
          <div className="sm:col-span-2 md:col-span-5 relative"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, category, or description..." className="w-full bg-background border border-border pl-9 pr-3 py-3 md:py-2 text-sm focus:outline-none focus:border-primary" data-testid="filter-search" /></div>
          <select value={category} onChange={(e) => setCategory(e.target.value)} className="sm:col-span-2 md:col-span-3 bg-background border border-border px-3 py-3 md:py-2 text-sm focus:outline-none focus:border-primary" data-testid="filter-category"><option value="">All categories</option>{categories.map((c) => <option key={c.category} value={c.category}>{c.category} ({c.count})</option>)}</select>
        ...
        {loading ? <div className="text-center py-16 text-muted-foreground text-xs uppercase tracking-[0.3em]">Loading inventory...</div> : listings.length === 0 ? <div className="text-center py-16 border border-border" data-testid="empty-state"><div className="font-display font-black text-2xl uppercase">No active listings right now</div><p className="text-muted-foreground text-sm mt-4">Be the first website owner to list banner inventory on BadAdz.</p><Link to="/register" className="inline-block mt-6 bg-primary text-primary-foreground px-6 py-3 text-xs uppercase tracking-[0.3em] font-bold hover:bg-acid hover:text-black transition-colors">List Your Site →</Link></div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" data-testid="listings-grid">{listings.map((listing) => <ListingCard key={listing.id} listing={listing} />)}</div>}
      </section>
...
function Stat({ label, value }) {
  return <div className="bg-background p-4"><div className="text-[9px] sm:text-[10px] uppercase tracking-[0.2em] sm:tracking-[0.25em] text-muted-foreground">{label}</div><div className="font-display font-black text-xl sm:text-2xl mt-1 text-foreground">{value}</div></div>;
}
