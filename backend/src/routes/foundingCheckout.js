...
const DEFAULT_COMMISSION_RATE = Number(process.env.PLATFORM_FEE_PERCENT || 20);
...
async function ensureCheckoutColumns() {
  await db.query(`
    ALTER TABLE users
    ...
    try {
      ...
      const { rows } = await db.query(
        `SELECT
           ...
           seller.founding_member AS seller_founding_member
         FROM listings l
      ...
      const unitAmount = Math.round(Number(listing.monthly_price) * 100);
      const stripe = getStripe();
      const frontend = process.env.FRONTEND_URL || 'http://localhost:3000';

      const sellerCommissionRate = Number(listing.seller_commission_rate || DEFAULT_COMMISSION_RATE);
      const total = Number(listing.monthly_price) * months;
      const platform_fee = +(total * (sellerCommissionRate / 100)).toFixed(2);
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        metadata: {
          ...
          seller_commission_rate: String(sellerCommissionRate),
          seller_founding_member: String(Boolean(listing.seller_founding_member)),
        },
      });
      ...
      console.error('founding checkout session error', err);
